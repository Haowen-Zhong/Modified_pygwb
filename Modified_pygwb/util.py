...
# NEW FUNCTION
def coarse_grain_window_factors(M, N, window_fftgram_dict={"window_fftgram": "hann"}, overlap_factor=0.5):
    """
    Calculate window factors. By default for a hann window with 50% overlap.
    See arxiv 2606.23816 for details.
    ...
    scipy.signal.get_window
        More information `here <https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.get_window.html>`_.
    """
    # compute w1w2curlybarsquared
    def kernel(d_array, M, N):
        num = np.sinc(d_array * M / (2*N)) 
        den = np.sinc(d_array / (2*N)) 
        return (num / den)**2 

    def compute_sum_with_shift(w, O, M):
        """
        Compute
          sum_{j=(1-O)N}^{N-1} sum_{k=(1-O)N}^{N-1}
             w[j]*w[j - (1-O)N]* w[k]*w[k - (1-O)N] * K(j-k; M)

        parameters:
          w : 1D array, len = N
          O : float, to compute shift = (1-O)*N
          M : used in kernel(d, M, N)

        return:
          the double summation result
        """
        N = len(w)
        shift = int(round((1 - O)*N))
        length = 2*N   
        X = np.zeros(length, dtype=float)

        start = max(0, shift)       
        end   = min(shift+N, length) 
        for n in range(start, end):
            j = n                
            j_minus = j - shift  
            if 0 <= j < N and 0 <= j_minus < N:
                X[n] = w[j]* w[j_minus]

        R = np.correlate(X, X, mode='full')
        idx = d_array + (length - 1)
        kd = kernel(d_array, M, N)
        Rd = R[idx]
        val = np.sum(Rd * kd)
        return val

    # The results are not sensitive to N, so one can precompute these numbers!
    # Use use N=16384 here
    if M==2 and window_fftgram_dict=={"window_fftgram": "hann"} and overlap_factor == 0.5:
        w1w2curlybarsquared = 0.12793878383771046
        w1w2curlyovlbarsquared = 0.01500853350835192
    elif M == 12 and window_fftgram_dict=={"window_fftgram": "hann"} and overlap_factor == 0.5:
        w1w2curlybarsquared = 0.041115231854855105
        w1w2curlyovlbarsquared = 0.0066149463588630365
    elif M == 30 and window_fftgram_dict=={"window_fftgram": "hann"} and overlap_factor == 0.5:
        w1w2curlybarsquared = 0.017515269513925503
        w1w2curlyovlbarsquared = 0.002932443451678957
    else:   
        if N >= 16384:
            N = 16384
        window_tuple = get_window_tuple(window_fftgram_dict)
        ...
        R = np.correlate(w**2, w**2, mode='full')
        d_array = np.arange(-(N-1), N)  # 长度 2N-1，对应 R 的各索引
        f_vals = kernel(d_array, M, N)
        w1w2curlybarsquared = np.sum(R * f_vals)/N**2
        if overlap_factor == 0.0 or overlap_factor == 0:
            w1w2curlyovlbarsquared = 0.0
        else:
            w1w2curlyovlbarsquared = 1 / (N*overlap_factor)**2 * compute_sum_with_shift(w, overlap_factor, M)

    return w1w2curlybarsquared, w1w2curlyovlbarsquared
