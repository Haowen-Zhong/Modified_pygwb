...
from .postprocessing import (
    ...
    postprocess_Y_sigma,  # Old name but different function
    postprocess_Yf_sigmaf # New function
)
...
class Baseline(object):
    ...
    def set_point_estimate_sigma_spectrum(
        ...
        # original: postprocess_Y_sigma now: postprocess_Yf_sigmaf
        # same inputs
        # we should combine frequency only
        point_estimate, sigma = postprocess_Yf_sigmaf(
    ...
    def set_point_estimate_sigma(
        ...
        if self._point_estimate_spectrum_set:
            ...
            # we need to keep this to calculate spectrograms and spectra
        ...
        if self.sigma_spectrum[0] == np.infty:
                ...
                return 
        # We no longer combine spectra, but compute this directly 
        bad_times_indexes = self._get_bad_times_indexes(times=self.point_estimate_spectrogram.times.value, badtimes=badtimes, apply_dsc=apply_dsc)
        # One needs to compute deltaF here.
        deltaF = self.frequencies[1] - self.frequencies[0]
        Y, sigma = postprocess_Y_sigma(
            self.point_estimate_spectrogram.value,
            self.sigma_spectrogram.value ** 2,
            self.duration,
            deltaF,
    ...
    def save_point_estimate_spectra(
        ...
        # 这里值得关注的一点是我们从来没有计算过单一segment对应的
        save(
            f"{filename}{ext}",
            self.frequencies,
            self.frequency_mask,
